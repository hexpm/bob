...
| `sort` | Comma-separated descending sort keys in precedence order: `elixir_version`, `erlang_version`, `os`, `os_version`, or `built_at`. Defaults to `built_at`. Version keys use numeric-aware ordering. Elixir and Erlang put a stable release before prereleases of the same release. `built_at` is exclusive; including it ignores every other sort key. Build time and ID break remaining ties. |
| `offset` | Number of matching records to skip. Defaults to `0`. |
