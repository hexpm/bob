...
aws s3 cp ${ref_name}.tar.gz s3://s3.hex.pm/builds/otp/${linux}/${ref_name}.tar.gz --cache-control "public,max-age=3600" --metadata "{\"surrogate-key\":\"builds builds/otp builds/otp/${linux} builds/otp/${linux}/${ref_name}\",\"surrogate-control\":\"public,max-age=604800\"}"
